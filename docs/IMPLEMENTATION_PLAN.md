...
- складские таблицы производимых позиций вместо ручной корректировки остатка запускают производство без плана: backend находит единственный активный рецепт, показывает поддерево, рекурсивно использует имеющиеся полуфабрикаты, производит недостающие и одной транзакцией проводит все списания и приходы;
- операции всего рекурсивного дерева агрегируются и при проведении создают факты работы: с выбранным сотрудником либо анонимно; тот же сценарий доступен на отдельной вкладке карточки производимого узла Canvas;
- для полуфабриката допускается рецепт-поддерево активного продуктового техпроцесса или собственный активный процесс; несколько активных поддеревьев блокируют производство как неоднозначное.
...
- сотрудник имеет тип оплаты `piecework/hourly`; для почасового обязательна ставка за час, работа вводится затраченными минутами, начисление считается по часовой ставке, а эквивалент выполненных операций — по норме времени;
- payroll отдельно показывает выполненный и оплаченный эквивалент операций, включая пропорциональный эквивалент частично оплаченной работы; тип и ставка сохраняются snapshot-полями факта работы.
