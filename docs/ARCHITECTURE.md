...
Реализованы каталоги и два складских ledger, версионируемые технологические процессы, детерминированное производственное планирование и атомарное исполнение планов. Потребности материалов/позиций/операций являются проекциями активных планов; выполненные сотрудниками работы и payroll остаются следующей доменной границей.
...
Поля операций `required_quantity` и `required_time_minutes`, а также складские `required/deficit/to_produce`, являются read-only проекциями активных планов. `completed_quantity` и агрегаты сотрудника пока равны нулю до этапа учёта работ/payroll.
...
Любая схема изменяется только Alembic. Миграции `0001–0005` создают каталоги, оба ledger и versioned техпроцессы; `0006` добавляет планы и снимки потребностей; `0007` — производственные записи, idempotency и строгие ссылки складских движений. PostgreSQL запускается Docker Compose; backend/frontend работают на host для быстрого reload/HMR. Production предполагает same-origin reverse proxy и отдельное object storage.
