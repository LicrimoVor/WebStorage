...
Реализованы каталоги и два складских ledger, версионируемые технологические процессы, детерминированное производственное планирование, атомарное исполнение планов, учёт выполненных работ и payroll. Потребности материалов/позиций/операций являются проекциями активных планов, а фактическое выполнение и расчёты с сотрудниками — проекциями work/payment ledger.
...
Любая схема изменяется только Alembic. Миграции `0001–0005` создают каталоги, оба ledger и versioned техпроцессы; `0006` добавляет планы и снимки потребностей; `0007` — производственные записи, idempotency и строгие ссылки складских движений; `0008` — выполненные работы, выплаты и allocation ledger. PostgreSQL запускается Docker Compose; backend/frontend работают на host для быстрого reload/HMR. Production предполагает same-origin reverse proxy и отдельное object storage.
