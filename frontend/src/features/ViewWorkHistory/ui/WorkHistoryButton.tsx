...
function EditWorkEntryButton({entry}: {entry: WorkEntry}) {
  ...
  const mutation = useMutation({
    mutationFn: (payload: WorkEntryUpdate) => updateWorkEntry(entry.id, payload),
    onSuccess: async () => {
      await Promise.all([
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={close} onEnterKeyDown={submit} maxWidth="s" fullWidth>
        ...
        <Dialog.Body>
          <div className={styles.form}>
            ...
            <Select
              ...
              options={modeOptions}
              value={[mode]}
              onUpdate={(values) => setMode((values[0] as WorkInputMode) ?? mode)}
              width="max"
              size="l"
            />
            <TextInput
              ...
              value={value}
              onUpdate={setValue}
              controlProps={{inputMode: 'decimal', 'aria-label': 'Объём работы'}}
              size="l"
            />
...
export function WorkHistoryButton({operation, employee}: WorkHistoryButtonProps) {
  const [open, setOpen] = useState(false);
