...
Семь законченных vertical slice внутренней производственной системы: склад материалов/полуфабрикатов/продуктов, справочники операций и персонала, версионируемые технологические процессы с Canvas-редактором, планирование и атомарная регистрация выпуска. Планы рекурсивно рассчитывают потребности, а факт производства списывает компоненты, приходует результат и обновляет прогресс одной транзакцией.
...
`time_norm` хранится как decimal-число минут на одну операцию, `price_per_operation` — как денежное decimal-значение. При записи выполненной работы оба значения фиксируются историческим snapshot. Начисление считается по количеству либо по эквиваленту `затраченные минуты / норма`; если норматив отсутствует, факт работы сохраняется без ложного расчёта. Выплата по умолчанию распределяется по старейшим неоплаченным начислениям (FIFO), также доступно точное ручное распределение.
